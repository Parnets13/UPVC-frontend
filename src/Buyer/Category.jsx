...
const DesignCard = ({ design }) => {
  return (
    <div className="bg-white shadow-md rounded-2xl p-6 relative flex flex-col justify-between min-h-[750px]">
      {/* SELECT Button */}
      <Link to="/select">
        <button className="absolute top-4 right-4 bg-black text-white text-xs font-semibold px-4 py-1 rounded-full hover:bg-gray-800 transition">
          SELECT
      ...
      <div className="mb-4">
        <h2 className="text-xl font-bold font-poppins uppercase mb-2">{design.name}</h2>
        <p className="text-gray-600 text-sm">{design.description}</p>
      </div>

      {/* Video */}
      <div className="w-full flex-grow bg-black rounded-lg overflow-hidden mb-4 min-h-[100px]">
        <video
          src={`http://localhost:9000${design.videoUrl}`}
      ...
      {/* Feature and Benefit Lists (optional - can be included later if backend provides it) */}
    </div>
  );
};
  ...
  return (
    <div className="mt-24">
      ...
      <div className="text-center max-w-2xl mx-auto mb-10 mt-24">
        ...
        <p className="text-gray-600 text-sm">
          Explore individual uPVC window designs curated across categories and brands.
        </p>
      </div>

      {/* Design Cards */}
      <div className="max-w-7xl mx-auto py-12 px-4 grid grid-cols-1 md:grid-cols-2 gap-8 font-sans">
